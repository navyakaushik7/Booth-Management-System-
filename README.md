...
login system (OTP over SMS) instead of the old fake demo login. Below is
...
get a one-time code by text message, and then can see/manage voter data
just for their own ward. Nobody can see another ward's data — that's
...
|---|---|---|
| Login | Fixed username/password, made up | Phone number → OTP texted to you → verify |
...
backend/                  Node.js + Express API
  ...
  src/utils/sms.js          sends the OTP via Twilio (if not set up, just prints it to the terminal instead)
...
## About the OTP / SMS login

While you're developing, you don't need a real Twilio account. If there
are no Twilio credentials set in `.env`, the OTP code just gets printed to
the backend terminal, AND it's sent back in the API response as
`dev_otp` — the login page will actually show this code on screen for you,
so you can log in without needing a real phone.

**To make it send real text messages:**
...
2. Add these to `backend/.env`:
   ...
   ```
3. Restart the backend. Once those are set, `dev_otp` stops showing up and
   real texts go out instead.
...
**Important:** these are placeholder numbers for testing only. Before this
...
I got `npm run build` to complete with zero errors, and every backend
route file passes `node --check` (basically a syntax check). **I could
not get Postgres/Docker running inside my sandbox environment**, so I was
not able to actually click through a real login → OTP → dashboard flow
end-to-end myself. Please run through the Quick Start steps above and let
me know if anything breaks — I'd rather find out now than after this is
actually being used.

## Security stuff I made sure to do (but please double-check)

- OTP codes are hashed with bcrypt before being stored — never saved as
...
- **Please change `JWT_SECRET` in `.env` to a long random value** before
  this touches any real data — don't leave it as the example value.
- This should sit behind HTTPS in production. Tokens and OTP codes should
  never be sent over plain HTTP.
